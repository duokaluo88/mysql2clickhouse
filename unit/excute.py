...
from mysql2clickhouse.model import conn
from mysql2clickhouse.model import parser

from mysql2clickhouse.unit import getMeta
...
class excuteFactory():
    ...
    def clickhouse_excute(self,sqllist):
        ck_conn = conn.clickhouse_conn(self.targetUser, self.targetPassword, self.targetHost, self.targetPort)
        for sql in sqllist:
            ck_conn.execute(sql)
        ck_conn.close()
