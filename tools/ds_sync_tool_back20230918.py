...
class mysql_conn:
    ...
    def get_ds_process_definition(self, processCode):
        ...
        sqlstr = "INSERT INTO dolphinscheduler.t_ds_process_definition " \
                 "(id, code, name, version, description, project_code, release_state, user_id, global_params, flag, locations, warning_group_id, timeout, tenant_id, execution_type, create_time, update_time)" \
    ...
    def get_ds_process_task_relation(self,processCode):
        ...
        for sqlresult in results:
            ...
            sqlstr = "INSERT INTO dolphinscheduler.t_ds_process_task_relation " \
                     "(id, name, project_code, process_definition_code, process_definition_version, pre_task_code, pre_task_version, post_task_code, post_task_version, condition_type, condition_params, create_time, update_time)" \
                     " VALUES%s ;" % str(tuple(ss)).replace('None', 'NULL')
            sqllist.append(sqlstr)
        return sqllist
    ...
    def get_ds_process_task_definition(self,processCode):
        ...
        for sqlresult in results:
            ...
            sqlstr = "INSERT INTO dolphinscheduler.t_ds_task_definition " \
                     "(id, code, name, version, description, project_code, user_id, task_type, task_params, flag, task_priority, worker_group, environment_code, fail_retry_times, fail_retry_interval, timeout_flag, timeout_notify_strategy, timeout, delay_time, resource_ids, create_time, update_time)" \
                     " VALUES%s ;" % str(tuple(ss)).replace('None', 'NULL')
            sqllist.append(sqlstr)
        return sqllist
    ...
    def get_ds_process_definition_log(self, processCode):
        ...
        sqlstr = "INSERT INTO dolphinscheduler.t_ds_process_definition_log " \
                 "(id, code, name, version, description, project_code, release_state, user_id, global_params, flag, locations, warning_group_id, timeout, tenant_id, execution_type, operator, operate_time, create_time, update_time)" \
                 " VALUES%s ;" % str(tuple(ss)).replace('None', 'NULL')
        sqllist.append(sqlstr)
        return sqllist
    ...
    def get_ds_schedules(self,processCode):
        ...
        sqlstr = "INSERT INTO dolphinscheduler.t_ds_schedules " \
                 "(id, process_definition_code, start_time, end_time, timezone_id, crontab, failure_strategy, user_id, release_state, warning_type, warning_group_id, process_instance_priority, worker_group, environment_code, create_time, update_time)" \
                 " VALUES%s ;" % str(tuple(ss)).replace('None', 'NULL')
        sqllist.append(sqlstr)
        return sqllist
...
class sql_parse:
    ...
    def sql_replace(self, sql_template):
        ...
        datepat=re.compile(r'([a-zA-Z0-9]+)_([a-zA-Z0-9]+)_integrate')
...
if __name__ == '__main__':
    ...
    config = {
        ...
        # 正则表达式替换  例如：e3plus_support_integrate -> e3plus-support-new-dev
        'DbName_Re':'\\1-\\2-dev',
    }
